...
prd = copy(JSON3.read(JSON.json(CSV.File("src/inputs/RILAs/Allianz/alz_iai_guard_rising.csv") |> DataFrame)))
...
Alz_IAI_Guard_Rising = Dict(:Alz_IAI_Guard_Rising => merge(prd, gf, yf, f, s, w, 
    rfg, rfy))
